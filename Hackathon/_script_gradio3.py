...
def check_age_gender(video):
    ...
    def extract_audio_from_video(video, target_sr=16000):
        video = VideoFileClip(video)
        audio_path = "temp_audio.wav"
        video.audio.write_audiofile(audio_path, fps=target_sr)
        waveform, sr = torchaudio.load(audio_path)
        waveform = torchaudio.transforms.Resample(orig_freq=sr,
                                                  new_freq=target_sr)(waveform)
    ...
    return f"Predicted age and gender: {output}"
...
def deepfake_detector(image, video):
    ...
    if isinstance(video, str):
        ...
        First_Message["Third test - Audio"] = Source_check_result[1]
        if Audio_check[0] != "Proceeding to next step.":
            return Audio_check[0]

    First_Message["Third test - Deepfake detection"] = "No0t c"
    return First_Message
...
if __name__ == "__main__":
    iface.launch(server_name="40.66.41.130", server_port=7860, share=True)
